...
func (c *PayloadController) Index() {
	...
	schedulerId := models.NewScheduler().GetMaxSchedulerId()
	...
	for _, scheduler := range schedulerList {
		...
		tasks.Add(tasks.Task{SchedulerId: schedulerId, ServerId: scheduler.ServerId, WebHookId: scheduler.WebHookId})
	}
